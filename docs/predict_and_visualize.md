# Prediction and Visualization with DPOcc
...
```bash
bash tools/dist_test.sh projects/configs/DPOcc_kitti/DPOcc_kitti.py $YOUR_CKPT 8 --pred-save $YOUR_SAVE_PATH
```

To inference with 8 GPUs on the validation set with all image frames, run:
```bash
bash tools/dist_test.sh projects/configs/DPOcc_kitti/DPOcc_kitti_video_demo.py $YOUR_CKPT 8 --pred-save $YOUR_SAVE_PATH
```
...
```bash
bash tools/dist_test.sh projects/configs/DPOcc_nusc/DPOcc_nusc_r50_256x704.py $YOUR_CKPT 8 --pred-save $YOUR_SAVE_PATH
```
...
```bash
bash tools/dist_test.sh projects/configs/DPOcc_nusc/DPOcc_nusc_r50_256x704.py $YOUR_CKPT 8 --pred-save $YOUR_SAVE_PATH --cfg-options data.test.ann_file=data/nuscenes_infos_temporal_val_visualize.pkl
```
...
The predicted results of DPOcc, MonoScene, and TPVFormer will be released for qualitative comparison. Please stay tuned.