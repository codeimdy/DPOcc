...
## SemanticKITTI
Following the practice from [MonoScene](https://github.com/astra-vision/MonoScene/issues/54), we train DPOcc only on the train sequences and select the checkpoint with best validation performance for test submission. This is how we get the benchmark results in the paper.

To prepare the test submission, we first inference DPOcc on test sequences:
```bash
bash tools/dist_test.sh projects/configs/DPOcc_kitti/DPOcc_kitti_submit.py $YOUR_CKPT 8 --test-save $YOUR_SAVE_PATH
```
...
First, we train DPOcc with train+val scenes:
```bash
bash tools/dist_train.sh projects/configs/DPOcc_nusc/DPOcc_nusc_r101_896x1600_trainval.py 8
```

Second, we inference DPOcc on test scenes with the final checkpoint:
```bash
bash tools/dist_test.sh projects/configs/DPOcc_nusc/DPOcc_nusc_r101_896x1600_trainval.py work_dirs/DPOcc_nusc_r101_896x1600_trainval/latest.pth 8 --test-save $YOUR_SAVE_PATH
```
