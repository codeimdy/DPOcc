...
def make_video(result_folder, video_file):
	...
	for index, image_file in enumerate(tqdm(os.listdir(result_folder))):
		...
		cv2.putText(img, 'DPOcc(ours)', (230, 640), font, font_size, (0, 0, 0), thickness)
...
def main_draw():
	...
	samples = [x for x in os.listdir(dpocc_path) if x[:2] == '00']
	samples.sort()
	print('found total {} samples'.format(len(samples)))

	for sample in samples:
		sample_id = sample.split('.')[0]
		dpocc_sample = os.path.join(dpocc_path, sample)
		...
		# load dpocc outputs
		with open(dpocc_sample, "rb") as handle:
			dpocc_data = pickle.load(handle)

		fov_mask = np.ones(dpocc_data['output_voxel'].shape[0]).astype(bool)
		dpocc_pred = dpocc_data['output_voxel'].reshape(256, 256, 32)
		input_image = dpocc_data['raw_img'][..., [2, 1, 0]] # BGR to RGB

		# draw dpocc predictions
		dpocc_img = draw(dpocc_pred, None, vox_origin, fov_mask, 
		...
		dpocc_img = Image.open(dpocc_img)
		occ_raw_w, occ_raw_h = dpocc_img.size
		top_crop = 300
		bottom_crop = 200
		dpocc_img = dpocc_img.crop([0, top_crop, occ_raw_w, occ_raw_h - bottom_crop])

		occ_raw_w, occ_raw_h = dpocc_img.size
		occ_h = int(resize_img_size[0] / occ_raw_w * occ_raw_h)
		occ_img_size = [resize_img_size[0], occ_h]
		dpocc_img = dpocc_img.resize(occ_img_size, Image.BILINEAR)
		...
		result.paste(dpocc_img, box=(0, resize_img_size[1] + spacing))
