...
class OccupancyEncoder(BaseModule):
    def __init__(
        ...
        for i in range(num_stage):
            # Conv3d uses the same stride on X/Y/Z, so the height visible to
            # every block in this stage is determined by the first block.
            stage_voxel_height = (voxel_height + block_strides[i] - 1) \
                // block_strides[i]
            self.layers.append(self._make_layer(
                ...
                norm_cfg=norm_cfg,
                stage_index=i,
                voxel_height=stage_voxel_height,
                first_pillar_stage=(i == 0),
                **kwargs,
            ))
